...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (existingAsset) {
            const fileUrl = pb.getFileUrl(existingAsset, existingAsset.file);
            return NextResponse.redirect(fileUrl.toString());
        }

        // 2. Si no existe, lo generamos
        ...
        // CORRECCIÓN: Convertir Uint8Array a un Buffer de Node.js antes de crear el Blob.
        ...
        console.error(`Error en el proceso de PDF para el documento ${params.id}:`, error);
        if (error.response) {
